...
2. **Relative Addressing:** The extensive use of Program Counter-relative jumps () for local control flow.
...
To overcome the limitations of v1, version v2 introduced the decomposition of code into **LCSAJ** (Linear Code Sequence and Jump). The object code was modeled as a directed graph , where each node  represents a Basic Block (BB) terminating in a jump or return instruction. The algorithm constructed a Reverse Graph () to enable **Backward Traversal** starting from "Sink" nodes (`ret`, `jr`, `jalr`).

**Critical Flaw ("The Disconnected Graph"):**
The initial implementation imposed an overly stringent adjacency constraint. An edge  was created between a source block  and a destination block  if and only if the jump target address in  matched the exact **Leader** (start address) of :
...
A modified *Breadth-First Search* ("Rainbow BFS") was implemented to manage path state. To mitigate combinatorial explosion, a **Frequency-Based Pruning** factor (, or "Darkness") was introduced. Given a node , if the visitation count  during the current exploration exceeds the threshold , the branch is pruned:
...
**Semantic Scoring:**
To qualify the results, a heuristic ranking system was developed based on exploit utility. Each gadget is assigned a score , calculated as a weighted sum of its properties:

Control of the Program Counter () and the first function argument () are assigned maximum weights (+50, +40), while chain length () introduces a penalty.
